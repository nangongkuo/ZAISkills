...
| 7 | 飞书文档发布（流转） | 拼多多 7.86.0 | dry-run 验证 | ✅ 已验证 |
...
| 飞书文档同步 | 手动复制粘贴，约 10 分钟 | `publish.sh` 系统推进，首次/后续幂等 | **20x** |
...
| 飞书文档幂等发布 | ❌ | ❌ | ✅ | document_id/url 记忆，首次 create 后 overwrite |
...
| 飞书文档发布 | 手动粘贴 | `publish.sh` 幂等同步 | 自动化 |
...
| 7 | 飞书文档发布 | `publish.sh` 幂等 create/overwrite | ✅ | 已通过 lark-cli docs dry-run 验证，document_id/url 记忆模型正常 |
